...
import admin_main_window as amw
...
def Login_button(root, entry1, entry2):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            tier = data["permissions"]
            root.destroy()
            if tier == 1:
                amw.admin_main_windows()
            else:
                mw.main_window(data["username"], tier)
        else:
...
def registration_form():
    ...
    role_var.set("elev")
    role_menu = tk.OptionMenu(window, role_var, "elev", "parinte")
